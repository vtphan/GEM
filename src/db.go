...
func create_tables() {
	...
	execSQL("create table if not exists back_feedback (id integer primary key, student_id integer, problem_id integer, feedback string, given_at timestamp)")
...
func init_database(db_name string) {
	...
	AddBackFeedbackSQL = prepare("insert into back_feedback (student_id, problem_id, feedback, given_at) values (?, ?, ?, ?)")
