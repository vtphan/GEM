...
func create_tables() {
	...
	execSQL("create table if not exists back_feedback (id integer primary key, feedback_id integer, feedback string, given_at timestamp)")
...
func init_database(db_name string) {
	...
	AddBackFeedbackSQL = prepare("insert into back_feedback (feedback_id, feedback, given_at) values (?, ?, ?)")
